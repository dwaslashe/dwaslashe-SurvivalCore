...
public class TeleportManager {
        ...
        teleportMap.put(user, new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (start.getBlockX() != current.getBlockX() || start.getBlockY() != current.getBlockY() || start.getBlockZ() != current.getBlockZ()) {
                    user.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &#FF3131Teleportacja została przerwana.. Poruszono się! &8<<"));
                ...
                if (toEnd <= 0) {
                    ...
                    user.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &#8dfa52Teleportacja przebiegła pomyślnie! &8<<"));
                    teleportMap.remove(user);
                    this.cancel();
                    return;
                }
                toEnd -= 1;
                user.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &#8dfa52Teleportacja nastapi za &#ffd56c{seconds}sek &#8dfa52do konca! &8<<".replace("{seconds}", toEnd + "")));
            }
    ...
    protected static void removeTeleport(Player user){
        ...
        teleportMap.remove(user);
        user.sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor("&8>> &#FF3131Teleportacja została anulowana! &8<<"));
    }
}

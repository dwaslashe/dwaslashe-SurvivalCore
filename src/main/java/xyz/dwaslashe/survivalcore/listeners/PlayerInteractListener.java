...
import xyz.dwaslashe.survivalcore.commands.LevelCommand;
...
public class PlayerInteractListener implements Listener {
    ...
    public static void openGui(int guiID, Player player, Player secondPlayer) {
        ...
        if (guiID == 0) {
            ...
            String level = PlaceholderAPI.setPlaceholders(secondPlayer, "%clv_player_level%");
            ItemStack information = inventoryHelper.prepareItemStack(Material.PLAYER_HEAD, itemStack -> {
                inventoryHelper.editSkullMetaForItemStack (itemStack, itemMeta -> {
                    ...
                    itemMeta.setDisplayName(Api.fixColor(ChatApi.getPrefix(secondPlayer).replace("<", "").replace(">", "") + secondPlayer.getName()));
                    itemMeta.setLore(PlaceholderAPI.setPlaceholders(secondPlayer, Api.fixColor(Arrays.asList(
                            "",
                            " &#E7E7E7Średnia ocena profilu: " + colorAverage(user.getRates()) + "&#65f654/&#54f5425",
                            ...
                            " &#E7E7E7Śmierci: &#b3b6ba%statistic_deaths% &f%img_skull_01a%",
                            " &#E7E7E7Zabójstwa: &#d42626%statistic_player_kills% &f%img_sword_03e%",
                            " &#E7E7E7Przegrane godziny: &#ffd56c%statistic_hours_played%g &fᎠ",
                            " &#E7E7E7Wykopane bloki: &#10F70C%statistic_mine_block% &#09b106⛏",
                            " &#E7E7E7Punkty rankingu: &#4eed6e%deluxecombat_ranking_points%pkt",
                            " &#E7E7E7Ilość powitanych nowych graczy: &#8eeb6c%Greeter_amount%",
                            " &#E7E7E7Pierwsza data dołączenia: &#ffd56c%player_first_join_date% &fᎠ",
            ...
            ItemStack hook = inventoryHelper.prepareItemStack(Material.BELL, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    ...
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &f᎘ &#FBFD8CAby zaczepić gracza")));
                });
            });

            ItemStack tradeinvite = inventoryHelper.prepareItemStack(Material.EMERALD, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    itemMeta.setDisplayName(Api.fixColor("&#4efc03Zaproponuj wymianę"));
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &f᎘ &#FBFD8CAby zaproponować wymiane")));
                });
            });
            ...
            ItemStack sendmsg = inventoryHelper.prepareItemStack(Material.WRITABLE_BOOK, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    ...
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &f᎘ &#FBFD8CAby napisać prywatną wiadomość!")));
                });
            });
            ...
            ItemStack rate = inventoryHelper.prepareItemStack(Material.OAK_HANGING_SIGN, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    ...
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &#E7E7E7Pamiętaj, że oceny profilu nie możesz później zmienić!", "", " &f᎘ &#FBFD8CAby ocenić profil gracza")));
                });
            });
            ...
            ItemStack kiss = inventoryHelper.prepareItemStack(Material.HEART_POTTERY_SHERD, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    ...
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &f᎘ &#FBFD8CAby pocałować gracza")));
                });
            });
            ...
            inventoryHelper.click(e -> {
                ...
                if (e.getSlot() == 22) {
                    if (secondPlayer == player) {
                        Api.sendMessage(player, Main.pluginConfig.getMessages().getPrefixFail() + "&#fc2419Nie możesz zrobić tego sobie!");
                        return;
                    }

                    if (delayHook.containsKey(player) && delayHook.get(player) > System.currentTimeMillis()) {
                        Api.sendMessage(player, Main.pluginConfig.getMessages().getPrefixFail() + "&#fc2419Aby zaczepić gracza musisz poczekać &#fcb419{TIME} &fᎠ".replace("{TIME}", TimerApi.secondsToString(delayHook.get(player))));
                    ...
                    player.closeInventory();
                    Api.sendMessage(player, Main.pluginConfig.getMessages().getPrefixSuccess() + "&#4cf739Pomyślnie zaczepiłeś'aś gracza!");
                    secondPlayer.sendTitle(Api.fixColor("&#95eb34&lHej &f\uE214"), Api.fixColor("&f楸 &#4cf739Gracz &#fcb419" + player.getDisplayName() + "&#4cf739 zaczepił Cię! &f楸"));
                    ...
                    NotificationBuilder.of(NotificationBuilder.NotificationType.CHAT,
                            "&f楹 <hover:show_text:\"<yellow>Kliknij mnie!\"><click:suggest_command:/msg <nick> ><#4cf739>Kliknij w wiadomość aby napisać do gracza prywatną wiadomość!</click></hover>"
                    ...
                    LevelCommand.openGui(0, secondPlayer, player);
                }
            });

            inventoryHelper.setItem(19, information);
            inventoryHelper.setItem(22, hook);
            inventoryHelper.setItem(23, tradeinvite);
            inventoryHelper.setItem(31, rate);
            inventoryHelper.setItem(32, kiss);

            if (me.q1zz.discordrewards.user.UserManager.getDiscordAccountID(secondPlayer.getUniqueId()) != -1) {
                ItemStack discordConnect = inventoryHelper.prepareItemStack(Material.PAPER, itemStack -> {
                    inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                        itemMeta.setCustomModelData(11293);
                        itemMeta.setDisplayName(Api.fixColor("&#7289daOdebrana nagroda za Discord"));
                        itemMeta.setLore(Api.fixColor(Arrays.asList("", " &#E7E7E7Konto odebrało już nagrodę przez", " &#E7E7E7dołączenie na serwer discord! \uE1F3 &#7289da/discord")));
                    });
                });
                inventoryHelper.setItem(24, discordConnect);
            } else inventoryHelper.setItem(24, sendmsg);
            ...
            if(stackIconLevel != null) {
                ...
                inventoryHelper.setItem(33, rankBadge.getItemStack());
            }
        ...
        if (guiID == 1) {
            ...
            ItemStack one = inventoryHelper.prepareItemStack(Material.RED_CONCRETE, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    ...
                    itemMeta.setLore(Api.fixColor(Arrays.asList("", " &f᎘ &#FBFD8CAby ustawić ocenę")));
                });
            });
        ...
        //3
        if (guiID == 3) {
            InventoryHelper inventoryHelper = new InventoryHelper(player, "&fᵩɍ", 4);

            ItemStack yes = inventoryHelper.prepareItemStack(Material.PAPER, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
            ...
            ItemStack no = inventoryHelper.prepareItemStack(Material.PAPER, itemStack -> {
                inventoryHelper.editMetaForItemStack(itemStack, itemMeta -> {
                    itemMeta.setCustomModelData(11196);
            ...
            inventoryHelper.setItem(13, yes);
            inventoryHelper.setItem(31, no);

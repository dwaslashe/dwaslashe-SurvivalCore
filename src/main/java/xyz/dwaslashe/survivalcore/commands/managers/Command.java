...
public abstract class Command extends org.bukkit.command.Command {
    ...
    public boolean execute(CommandSender commandSender, String s, String[] strings) {
        ...
        if (commandSender instanceof ConsoleCommandSender) {
            ...
            if (!hasPermission()) {
                ((Player) commandSender).sendTitle(Api.fixColor(Main.pluginConfig.getMessages().getIp()), Api.fixColor(" &8>> &cNie posiadasz uprawnien &8(&e{permission}&8) &8<<".replace("{permission}", getPermission())));
            } else {
                commandExecute(commandSender, strings);
            }
        }

...
public class BossBarApi {
    ...
    public static void sendGlobal2(BarColor color, BarStyle style, int time, String message) {
        org.bukkit.boss.BossBar bar = Bukkit.createBossBar(message, color, style, new BarFlag[0]);
        bar.setProgress(0);
        Main.getPlugin().getUserCache().getOnlineUserMap().forEach((name, user) -> {
            UserImpl impl = (UserImpl) user;
    ...
    public static void sendGlobal3(BarColor color, BarStyle style, int time, String message) {
        org.bukkit.boss.BossBar bar = Bukkit.createBossBar(message, color, style, new BarFlag[0]);
        bar.setProgress(0);
        Main.getPlugin().getUserCache().getOnlineUserMap().forEach((name, user) -> {
            UserImpl impl = (UserImpl) user;
    ...
    public static void sendGlobalParse2(String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            if (!args[i].startsWith("color:") && !args[i].startsWith("c:")) {
                ...
                color = parseBarColor(args[i].replaceAll("(c|color):", ""));
                sendGlobal2(color, BarStyle.SOLID, 5, Api.fixColor(message.toString()));
                if (color == null) {
                    Api.sendLog("&cNie ma takiego koloru! [BossBar]");
                    return;
                }
    ...
    public static void sendGlobalParse3(String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            if (!args[i].startsWith("color:") && !args[i].startsWith("c:")) {
                ...
                color = parseBarColor(args[i].replaceAll("(c|color):", ""));
                sendGlobal3(color, BarStyle.SOLID, 5, Api.fixColor(message.toString()));
                if (color == null) {
                    Api.sendLog("&cNie ma takiego koloru! [BossBar]");
                    return;
                }
